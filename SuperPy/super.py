...
from functions.date import TODAY, set_date, change_date
from functions.buy_sell import BOUGHT_PATH, SOLD_PATH, buy_product, sell_product
from functions.inventory import Stock
from functions.plotreports import REPORTS_PATH, get_report, plot_df
...
args = functions.arguments.create_super_parser()
...
if args.command == "show-inventory":
    stock = Stock(BOUGHT_PATH, SOLD_PATH)
    df, headers = stock.show_inventory(args.date)
    print(
        tabulate(
            df,
            headers=headers,
...
if args.command == "show-product":
    product = Stock(BOUGHT_PATH, SOLD_PATH).show_product(args.product_id)
    print(
    ...
    if product.sold_id is not None:
        print(
            ...
            f"Quantity of product sold: {product.sell_quantity}\n",
            f"Product in inventory: {product.quantity - product.sell_quantity}",
        )
...
if args.command == "report-total":
    ...
    if args.type_of_report == "revenue":
        ...
        if args.type_of_report == "profit":
            if args.day:
                profit = stock.get_profit_day(args.day)
                print(f"Total profit on {args.day}: {profit}")
            elif args.month:
                profit = stock.get_profit_month(args.month)
                month = args.month.strftime("%B")
                print(f"Total profit in {month} {args.month.year}: {profit}")
            elif args.year:
                profit = stock.get_profit_year(args.year)
...
if args.command == "report-period":
    ...
    if args.type_of_report == "revenue":
        df = get_report(args.report_month, stock.get_revenue_day)
        df.columns = ["Day", "Revenue"]
    elif args.type_of_report == "profit":
        df = get_report(args.report_month, stock.get_profit_day)
        df.columns = ["Day", "Profit"]
    elif args.type_of_report == "revenue-profit":
        df_revenue = get_report(args.report_month, stock.get_revenue_day)
        df_profit = get_report(args.report_month, stock.get_profit_day)
        ...
        if args.product is not None:
            ...
            print(
                ...
                please enter '--product' followed by the product"""
            )
            sys.exit()

...
def get_report(report_month, report_method, product=None):
    ...
    for day in range(1, no_days_in_month + 1):
        if product is None:
            ...
            output = report_method(
                date(report_month.year, report_month.month, day), product
            )
...
def plot_df(df, overview_month):
    ...
    if df.columns[1] == "Revenue" or df.columns[1] == "Profit":
        title = f"""Daily {y_name}
        for {overview_month.strftime('%B')} {overview_month.year}"""
    else:
        title = f"""{y_name.capitalize()}
        per day for {overview_month.strftime('%B')} {overview_month.year}"""
